...
const DataAsetLain = ({ onBack }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setEditingId(null);
      setForm({ nama: '', nilai_perolehan: '', tanggal_perolehan: '', keterangan: '' });
    } catch (err) {
  ...
  const openEdit = (item) => {
    setForm({
      ...
      tanggal_perolehan: item.tanggal_perolehan?.split('T')[0] || '',
    ...
    <div className="bg-white p-6 rounded-xl shadow-md">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-gray-800">Data Aset Lain</h2>
        </div>
        <button
          onClick={openAdd}
            ...
            {data.map((item) => (
              <tr key={item.id} className="bg-white border shadow-sm rounded">
                ...
                <td className="px-4 py-2 text-right">
                  {parseFloat(item.nilai_perolehan).toLocaleString('id-ID')}
                </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-xl">
            <h2 className="text-lg font-semibold mb-4">{editingId ? 'Edit' : 'Tambah'} Aset Lain</h2>
