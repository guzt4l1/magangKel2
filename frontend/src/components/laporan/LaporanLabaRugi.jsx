// src/components/laporan/LaporanLabaRugi.jsx
...
const API = 'http://localhost:5000/api/laporan/laba-rugi';
...
const LaporanLabaRugi = ({ onBack }) => {
  ...
  const exportToExcel = () => {
    ...
    writeFile(wb, `Laporan_Laba_Rugi_${bulan}_${tahun}.xlsx`);
  };

  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(14);
    doc.text(`Laporan Laba Rugi - ${bulanList[bulan - 1]} ${tahun}`, 14, 15);
    ...
    doc.save(`Laporan_Laba_Rugi_${bulan}_${tahun}.pdf`);
  };
      ...
      {data ? (
        <div className="space-y-6">
          ...
          <div className="mt-4 text-sm font-medium">
            <div>Total Pendapatan: <span className="float-right">{Number(data.total_pendapatan).toLocaleString()}</span></div>
            <div>Total Beban: <span className="float-right">{Number(data.total_beban).toLocaleString()}</span></div>
            <div className="text-blue-700 font-semibold">Laba Bersih: <span className="float-right">{Number(data.laba_bersih).toLocaleString()}</span></div>
          </div>
        </div>
      ) : (
