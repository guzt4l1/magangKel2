...
const API = 'http://localhost:5000/api/penghasilan-lain';
const API_AKUN = 'http://localhost:5000/api/akun';

const DataPenghasilanLain = ({ onBack }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      akun_pendapatan_id: parseInt(form.akun_pendapatan_id),
  ...
  const openEdit = (item) => {
    setForm({
      ...
      akun_pendapatan_id: item.akun_pendapatan_id.toString(),
      jumlah: item.jumlah,
    ...
    <div className="bg-white p-6 rounded-xl shadow-md">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-gray-800">Data Penghasilan Lain</h2>
        </div>
        <button
          onClick={openAdd}
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id} className="bg-white border shadow-sm rounded">
                ...
                <td className="px-4 py-2">{getNama(akunList, item.akun_pendapatan_id)}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-xl">
            <h2 className="text-lg font-semibold mb-4">{editingId ? 'Edit' : 'Tambah'} Penghasilan Lain</h2>
            <form onSubmit={handleSubmit} className="space-y-4 text-gray-700">
              ...
              <select
                value={form.akun_pendapatan_id}
                onChange={(e) => setForm({ ...form, akun_pendapatan_id: e.target.value })}
                required
                className="w-full border px-3 py-2 rounded"
              >
...
export default DataPenghasilanLain;
