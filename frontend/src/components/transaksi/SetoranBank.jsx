...
const API = 'http://localhost:5000/api/setoran-bank';
const API_AKUN = 'http://localhost:5000/api/akun';

const SetoranBank = ({ onBack }) => {
  ...
  const getNamaAkun = (id) => {
    const item = akunList.find(a => a.id === id);
    ...
    <div className="bg-white p-6 rounded-xl shadow-md">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-gray-800">Data Setoran Bank</h2>
        </div>
        <button
          onClick={openAdd}
        ...
        <table className="w-full text-sm text-left text-gray-700 border-separate border-spacing-y-2">
          <thead className="bg-gradient-to-r from-blue-900 to-blue-600 text-white">
            <tr>
              ...
              <th className="px-4 py-2">Akun Bank</th>
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id} className="bg-white border shadow-sm rounded">
                ...
                <td className="px-4 py-2">{getNamaAkun(item.akun_bank_id)}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-xl">
            <h2 className="text-lg font-semibold mb-4">{editingId ? 'Edit' : 'Tambah'} Setoran Bank</h2>
