...
const API = 'http://localhost:5000/api/pengeluaran-beban';
const API_BEBAN = 'http://localhost:5000/api/beban-lain';
const API_AKUN = 'http://localhost:5000/api/akun';

const PengeluaranBeban = ({ onBack }) => {
  ...
  const fetchRelasi = async () => {
    try {
      const [resBeban, resAkun] = await Promise.all([
        axios.get(API_BEBAN),
        axios.get(API_AKUN)
      ]);
      setBebanList(resBeban.data);
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...form,
      beban_id: parseInt(form.beban_id),
      akun_beban_id: parseInt(form.akun_beban_id),
  ...
  const openEdit = (item) => {
    setForm({
      ...
      beban_id: item.beban_id.toString(),
  ...
  const getNama = (list, id, showKode = false) => {
    const item = list.find(i => i.id === id);
    if (!item) return '-';
    return showKode ? `${item.kode} - ${item.nama}` : item.nama;
  };
    ...
    <div className="bg-white p-6 rounded-xl shadow-md">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-gray-800">Data Pengeluaran Beban</h2>
        </div>
        <button
          onClick={openAdd}
        ...
        <table className="w-full text-sm text-left text-gray-700 border-separate border-spacing-y-2">
          <thead className="bg-gradient-to-r from-blue-900 to-blue-600 text-white">
            <tr>
              ...
              <th className="px-4 py-2">Beban</th>
              <th className="px-4 py-2">Akun Beban</th>
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id} className="bg-white border shadow-sm rounded">
                ...
                <td className="px-4 py-2">{getNama(bebanList, item.beban_id)}</td>
                <td className="px-4 py-2">{getNama(akunList, item.akun_beban_id, true)}</td>
                <td className="px-4 py-2">{getNama(akunList, item.akun_kas_id, true)}</td>
                <td className="px-4 py-2">{item.jumlah}</td>
                <td className="px-4 py-2">{item.bukti || '-'}</td>
            ...
            {data.length === 0 && (
              <tr>
                <td colSpan={8} className="text-center text-gray-500 py-4">Tidak ada data.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-xl">
            <h2 className="text-lg font-semibold mb-4">{editingId ? 'Edit' : 'Tambah'} Pengeluaran Beban</h2>
            <form onSubmit={handleSubmit} className="space-y-4 text-gray-700">
              ...
              <select
                value={form.beban_id}
                onChange={(e) => setForm({ ...form, beban_id: e.target.value })}
                required
                className="w-full border px-3 py-2 rounded"
              >
              ...
              <div className="flex justify-end gap-2">
                <button type="button" onClick={closeModal} className="px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded">
                  Batal
                </button>
...
export default PengeluaranBeban;
