...
const API = 'http://localhost:5000/api/penarikan-bank';
const API_AKUN = 'http://localhost:5000/api/akun';

const DataPenarikanBank = ({ onBack }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...form,
      akun_bank_id: parseInt(form.akun_bank_id),
  ...
  const openEdit = (item) => {
    setForm({
      ...
      akun_bank_id: item.akun_bank_id.toString(),
    ...
    <div className="bg-white p-6 rounded-xl shadow-md">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-gray-800">Data Penarikan Bank</h2>
        </div>
        <button
          onClick={openAdd}
        ...
        <table className="w-full text-sm text-left text-gray-700 border-separate border-spacing-y-2">
          <thead className="bg-gradient-to-r from-blue-900 to-blue-600 text-white">
            <tr>
              ...
              <th className="px-4 py-2">Dari Bank</th>
              <th className="px-4 py-2">Ke Kas</th>
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id} className="bg-white border shadow-sm rounded">
                ...
                <td className="px-4 py-2">{getNama(item.akun_bank_id)}</td>
                <td className="px-4 py-2">{getNama(item.akun_kas_id)}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-xl">
            <h2 className="text-lg font-semibold mb-4">{editingId ? 'Edit' : 'Tambah'} Penarikan Bank</h2>
            <form onSubmit={handleSubmit} className="space-y-4 text-gray-700">
              ...
              />
              <select
                value={form.akun_bank_id}
                onChange={(e) => setForm({ ...form, akun_bank_id: e.target.value })}
                required
                className="w-full border px-3 py-2 rounded"
              >
...
export default DataPenarikanBank;
