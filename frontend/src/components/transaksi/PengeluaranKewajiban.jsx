...
const API = 'http://localhost:5000/api/pengeluaran-kewajiban';
const API_AKUN = 'http://localhost:5000/api/akun';
const API_UTANG = 'http://localhost:5000/api/utang';

const DataPengeluaranKewajiban = ({ onBack }) => {
  ...
  const fetchRelasi = async () => {
    try {
      const [resAkun, resUtang] = await Promise.all([
        axios.get(API_AKUN),
        axios.get(API_UTANG)
      ]);
      setAkunList(resAkun.data);
      setUtangList(resUtang.data);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...form,
      utang_id: parseInt(form.utang_id),
  ...
  const openEdit = (item) => {
    setForm({
      ...
      utang_id: item.utang_id.toString(),
  ...
  const getNama = (list, id, isKode = false) => {
    ...
    return item.nama_pemberi || item.nama;
  };
    ...
    <div className="bg-white p-6 rounded-xl shadow-md">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-gray-800">Data Pengeluaran Kewajiban</h2>
        </div>
        <button
          onClick={openAdd}
        ...
        <table className="w-full text-sm text-left text-gray-700 border-separate border-spacing-y-2">
          <thead className="bg-gradient-to-r from-blue-900 to-blue-600 text-white">
            <tr>
              ...
              <th className="px-4 py-2">Utang</th>
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id} className="bg-white border shadow-sm rounded">
                ...
                <td className="px-4 py-2">{getNama(utangList, item.utang_id)}</td>
                <td className="px-4 py-2">{getNama(akunList, item.akun_kas_id, true)}</td>
                <td className="px-4 py-2">{item.jumlah_bayar}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-xl">
            <h2 className="text-lg font-semibold mb-4">{editingId ? 'Edit' : 'Tambah'} Pengeluaran Kewajiban</h2>
            <form onSubmit={handleSubmit} className="space-y-4 text-gray-700">
              ...
              <select
                value={form.utang_id}
                onChange={(e) => setForm({ ...form, utang_id: e.target.value })}
                required
                className="w-full border px-3 py-2 rounded"
              >
                <option value="">Pilih Utang</option>
                {utangList.map(u => (
                  <option key={u.id} value={u.id}>{u.nama_pemberi} - {u.jumlah}</option>
                ))}
              </select>
              <select
...
export default DataPengeluaranKewajiban;
